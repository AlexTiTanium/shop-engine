...
class Map {
   ...
   *
   */
  public function __construct(RouteFactory $route_factory, array $attach = null){
  ...
  public function attach($path_prefix, array $spec){
    ...
    if(!isset($spec['routes'])) {
      throw new SystemException("No routes defined for attached path prefix '$path_prefix'.");
    }
  ...
  protected function getNextAttach(){
    ...
    if(is_string($key) && is_string($val)) {
      ...
      throw new SystemException("Route spec for '$key' should be a string or array.");
    }
